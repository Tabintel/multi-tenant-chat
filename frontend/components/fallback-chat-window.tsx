...
interface FallbackChatWindowProps {
  channelId: string
  channelName?: string
}

export function FallbackChatWindow({ channelId, channelName }: FallbackChatWindowProps) {
  const [messages, setMessages] = useState<any[]>([])
  const [newMessage, setNewMessage] = useState("")
  const [currentUser, setCurrentUser] = useState({ name: "" })
  const messagesEndRef = useRef<HTMLDivElement>(null)

  const displayName = channelName || channelId.split("-").pop() || channelId
